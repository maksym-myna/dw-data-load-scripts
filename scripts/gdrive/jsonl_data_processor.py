...
class JSONLDataProcessor(DataProcessor):
    ...
    def run(self, directory=r"open library dump") -> None:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            list_of_file_lists = {
                executor.submit(parser.process_latest_file, directory)
                for parser in self.ol_parsers
            }
            list_of_file_lists.add(
                executor.submit(
    ...
    def upload_to_drive(service, files: list[str]) -> None:
        ...
        for file_location in files:
            ...
            media_body = MediaFileUpload(
                file_location, mimetype="application/json", resumable=True
            )
            file = search.get("files")[0]
            id = file.get("id")

            request = service.files().update(fileId=id, body={}, media_body=media_body)
            response = None
            ...
            service.revisions().delete(
                fileId=id, revisionId=versions["revisions"][0]["id"]
            ).execute()
