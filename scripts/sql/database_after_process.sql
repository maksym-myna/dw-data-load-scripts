...
CALL insert_work_and_subject('Neal Schuman Publishing', '9780375712364', 'eng', 'Brave New World', 469, 549, 2013, 'BOOK', 3, 'Aldous Huxley');
CALL insert_work_and_subject('Unknown', '9783596706648', 'eng', 'Ready Player One', 181, 2.036, 1952, 'BOOK', 4, 'Ernest Cline');
CALL insert_work_and_subject('Harlequin', '9780373031122', 'eng', 'An impossible passion', 187, 1.34, 1991, 'BOOK', 5, 'Stephanie Howard');
...
CALL insert_work_and_subject('Indypublish', '9781435327870', 'eng', 'St Ives', 260, 0.54, 2007, 'BOOK', 15, 'Louis Robert Stevenson');
CALL insert_work_and_subject('Unknown', '9781574531725', 'eng', 'Phoenix Rising: No-Eyes Vision of the Change to Come', 203, 0.36, 1997, 'BOOK', 16, 'Nancy Fish');
CALL insert_work_and_subject('Christian Herald Books', '9780915684687', 'eng', 'Bible Stories to Grow by', 202, 2.51, 1980, 'BOOK', 17, 'Mary Batchelor');
CALL insert_work_and_subject('Hodder', '9780340918616', 'eng', 'My sisters keeper', 364, 0.91, 2008, 'BOOK', 18, 'Jodi Picoult');
CALL insert_work_and_subject('Rentice Hall', '9780130998170', 'eng', 'Macroeconomics: Theories and Policies 6th Edition', 481, 1.2, 1998, 'BOOK', 19, 'Richar T. Froyen');
...
FROM 
    work
JOIN 
  work_author using (work_id)
JOIN 
  author using (author_id)
WHERE 
    author.full_name = 'Aldous Huxley'
    AND work.isbn IN (
        '9780375712364',
        '9798468471463',
        '9780006547280',
...
AS $$
BEGIN
    EXECUTE 'CREATE MATERIALIZED VIEW available_copies_tmp AS
    SELECT
      work_id,
      COUNT(ii) AS qty
    FROM
    ...
    BEGIN
        EXECUTE 'DROP MATERIALIZED VIEW available_copies';
        EXECUTE 'ALTER MATERIALIZED VIEW available_copies_tmp RENAME TO available_copies';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'An error occurred while refreshing the available_copies view: %', SQLERRM;
    END;
END;
$$;
